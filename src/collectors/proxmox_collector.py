# src/collectors/proxmox_collector.py
"""
Proxmox collector for gathering VM/LXC configurations, storage, and network settings.
"""
...
class ProxmoxCollector(SystemStateCollector):
    """
    ...
    def validate_config(self) -> bool:
        """Validate Proxmox collector configuration"""
        if not self.host:
            self.logger.error("Host required for Proxmox collection")
            return False
        return True

    def get_system_state(self) -> Dict[str, Any]:
        """Get Proxmox system state"""
        try:
            if not self.ssh_connector.connect():
                raise Exception("Failed to establish SSH connection")

            # Collect all Proxmox data
            ...
            self.logger.error(f"Failed to collect Proxmox system state: {e}")
    ...
    def _get_vm_configurations(self) -> List[Dict]:
        ...
        try:
            # Get list of VMs
            result = self.ssh_connector.execute_command("qm list")
            ...
            for line in vm_lines:
                if line.strip():
                    ...
                    if len(parts) >= 2:
                        ...
                        config_result = self.ssh_connector.execute_command(f"qm config {vmid}")
    ...
    def _get_lxc_configurations(self) -> List[Dict]:
        ...
        try:
            ...
            result = self.ssh_connector.execute_command("pct list")
            if not result.success:
            ...
            for line in lxc_lines:
                if line.strip():
                    ...
                    if len(parts) >= 2:
                        vmid = parts[0]
                        status = parts[1]
                        # LXC list format can vary, try to get name
                        name = ' '.join(parts[2:]) if len(parts) > 2 else f"ct-{vmid}"

                        # Get detailed LXC configuration
                        config_result = self.ssh_connector.execute_command(f"pct config {vmid}")
    ...
    def _get_storage_configuration(self) -> Dict[str, Any]:
        ...
        try:
            # Get storage status
            result = self.ssh_connector.execute_command("pvesm status")
            if result.success:
                storage_info['pvesm_status'] = self._parse_storage_status(result.output)

            # Get storage configuration from file
            result = self.ssh_connector.execute_command("cat /etc/pve/storage.cfg")
            if result.success:
    ...
    def _get_network_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.ssh_connector.execute_command("cat /etc/network/interfaces")
            if result.success:
                network_info['interfaces'] = {'raw_config': result.output}

            # Get bridge information
            result = self.ssh_connector.execute_command("brctl show")
            if result.success:
    ...
    def _get_cluster_information(self) -> Dict[str, Any]:
        ...
        try:
            # Get cluster status
            result = self.ssh_connector.execute_command("pvecm status 2>/dev/null || echo 'No cluster'")
            if result.success:
            ...
            if cluster_info.get('clustered', False):
                result = self.ssh_connector.execute_command("pvecm nodes")
                if result.success:
    ...
    def _get_node_information(self) -> Dict[str, Any]:
        ...
        try:
            # Get node status
            result = self.ssh_connector.execute_command("pvesh get /nodes/$(hostname)/status")
            if result.success:
                try:
            ...
            # Get Proxmox version
            result = self.ssh_connector.execute_command("pveversion")
            if result.success:
                node_info['version'] = result.output.strip()

            # Get subscription info
            result = self.ssh_connector.execute_command(
                "pvesubscription get 2>/dev/null || echo 'No subscription info'")
            if result.success and 'No subscription info' not in result.output:
                try:
                    node_info['subscription'] = json.loads(result.output)
                except json.JSONDecodeError:
                    node_info['subscription'] = {'raw': result.output}
    ...
    def sanitize_data(self, data: Any) -> Any:
        """Proxmox-specific data sanitization"""
        if isinstance(data, dict):
            sanitized = {}
            for key, value in data.items():
                # Sanitize sensitive Proxmox configuration
                if any(sensitive in key.lower() for sensitive in ['password', 'secret', 'token', 'key']):
                    sanitized[key] = 'REDACTED'
                elif key.lower() in ['subscription', 'license']:
                    # Sanitize subscription information
                    if isinstance(value, dict):
                        sanitized[key] = {k: 'REDACTED' if 'key' in k.lower() else v for k, v in value.items()}
                    else:
                        sanitized[key] = 'REDACTED'
                else:
