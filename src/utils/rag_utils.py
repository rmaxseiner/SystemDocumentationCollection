...
class TemporalDataCleaner:
    ...
    CONTAINER_TEMPORAL_FIELDS = {
        'status', 'state', 'running', 'restarting', 'paused', 'exited', 'dead',
        'started_at', 'finished_at', 'exit_code', 'oom_killed',
        'pid', 'process_list', 'top_output',
        'log_path', 'log_config', 'host_config.log_config',
        'network_settings.networks.*.ip_address',  # Runtime IPs change
        'network_settings.ports',  # Runtime port mappings
        'mounts.*.source',  # May contain runtime paths
        'health_check.status', 'health_check.failing_streak',
    ...
    def clean_data(self, data: Dict[str, Any], data_type: str) -> Dict[str, Any]:
        ...
        self.logger.debug(f"Cleaned {data_type} data: removed {len(data) - len(cleaned)} top-level fields")
        return cleaned
    
    def _get_removal_fields(self, data_type: str) -> Set[str]:
        """Get set of fields to remove for given data type"""
        fields = self.DEFAULT_TEMPORAL_FIELDS.copy()
        
        if data_type == 'container':
            fields.update(self.CONTAINER_TEMPORAL_FIELDS)
        elif data_type == 'host':
            fields.update(self.HOST_TEMPORAL_FIELDS)
        elif data_type == 'service':
            fields.update(self.SERVICE_TEMPORAL_FIELDS)
    ...
    def _should_remove_field(self, field_name: str, fields_to_remove: Set[str]) -> bool:
        """Check if field should be removed based on patterns"""
...
class MetadataExtractor:
    ...
    def _extract_container_metadata(self, data: Dict[str, Any], entity_id: str) -> Dict[str, Any]:
        ...
        # Environment variables (may contain relationships)
